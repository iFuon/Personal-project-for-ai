...
def train(config, epoch, num_epoch, epoch_iters, base_lr,
          num_iters, trainloader, optimizer, model, writer_dict, bench):
    # Training
    model.train()
    ...
    for i_iter, batch in enumerate(trainloader, 0):
        ...
        #optimizer.zero_grad()
        if bench >= 3:
            images = images.to(device="cuda", memory_format=torch.channels_last)
        else:
             # baseline
            images = images.cuda()
        labels = labels.long().cuda()
        ...
        if bench == 2 or bench == 3:
            ...
            model.zero_grad() # zero out the gradients so that you do the parameter update correctly.
            loss.backward()
            optimizer.step()
        else:
            #print("amp-------------------------------")
...
def testval(config, test_dataset, testloader, model,
    ...
    pos = confusion_matrix.sum(1) # return all necessary indicators
    res = confusion_matrix.sum(0)
    tp = np.diag(confusion_matrix)
